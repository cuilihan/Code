...
namespace DRP.BF.ResMrg
{
    ...
    public class Shopping_BF
    {
        ...
        /// <returns></returns>
        public List<DAL.Res_Shopping> QueryData(ResourceCriteria qry, out int record)
        {
            return db.GetPaginationList<DAL.Res_Shopping>("Res_Shopping", qry.pageIndex, qry.pageSize, out record, QueryCondition(qry), qry.SortExpress);
        }
        ...
        /// <returns></returns>
        public DAL.Res_Shopping Get(string keyID)
        {
            return DAL.Res_Shopping.SingleOrDefault(x => x.ID == keyID);
        }

        /// <summary>
        /// 保存酒店
        /// </summary> 
        public bool Save(DAL.Res_Shopping entity, List<DAL.Res_BizContact> list)
        {
            var user = AuthenticationPage.UserInfo;
            try
            {
            ...
            {
                BizUtility.ExceptionHandler(user, ex, "保存购物店时发生错误");
                return false;
            }
        ...
        public bool Delete(List<string> listID)
        {
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    listID.ForEach(x =>
                    {
                        ...
                        DAL.Res_Shopping.Delete(t => t.ID == x);
                    });
                    scope.Complete();
                    return true;
                }
            ...
            {
                BizUtility.ExceptionHandler(AuthenticationPage.UserInfo, ex, "删除购物店时发生错误");
                return false;
            }
        }
    }
