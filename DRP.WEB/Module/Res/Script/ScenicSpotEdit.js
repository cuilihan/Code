...
var c = {
    ...
    serverUrl: "Service/ScenicTicket.ashx",
    ...
    fnSaveData: function () { //保存
        ...
        var cooperatePrice = $("#CooperatePrice").val();
        var addr = $("#Addr").val();
        ...
        var json = { "KeyID": keyID, "RouteTypeID": routeTypeID, "DestinationID": destinationID, "Name": name, "IsEnable": isEnable, "Contact": contact, "Title": title, "Mobile": mobile, "Phone": phone, "Fax": fax, "Mail": mail, "NormalPrice": normalPrice, "TeamPrice": teamPrice, "CooperatePrice": cooperatePrice, "Addr": addr, "Comment": comment, "Item": items };
        var u = c.serverUrl + "?action=2&r=" + getRand();
        dataService.ajaxPost(u, json, function (data) {
            if (data == "1") {
                closeTab("保存成功", "景点门票维护");
            }
            else {
                Alert("保存失败");
            }
